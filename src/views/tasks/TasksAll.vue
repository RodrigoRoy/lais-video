  ...
  <div class="d-flex flex-column">
    ...
    <div v-if="tasks && tasks.length > 0" class="d-flex flex-wrap justify-content-start">
      <div v-for="task in tasks" v-bind:key="task._id">
        <b-card class="card mb-2 ml-2 text-white bg-dark" style="width: 18rem;">
      ...
      <div>
        <b-modal id="modal1" ref="modal" centered title="Delete Confirmation">
          ...
          <div slot="modal-footer" class="w-100 text-right">
            ...
            <b-btn slot="md" class="mr-1" variant="secondary" @click="cancelModal">Cancel</b-btn>
          </div>
        </b-modal>
      </div>
...
export default{
  ...
  methods: {
    ...
    markAsCompleted: async function(task){
      task.completed = true;
      const request = {
        task: task
      };
