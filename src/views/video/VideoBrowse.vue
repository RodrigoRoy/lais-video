    ...
    <v-dialog v-model="dialog" overlay-opacity="0.80">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <div v-if="video">
            <v-btn v-if="$store.state.active && $store.state.operation.update" @click="goToEdit(video)" class="mr-2">Editar <v-icon>mdi-pencil</v-icon> </v-btn>
            <v-btn v-if="$store.state.active && $store.state.operation.delete" @click="remove(video)" class="mr-2">Borrar <v-icon>mdi-delete</v-icon> </v-btn>
